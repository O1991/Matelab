...
while DIFF>tol  && ITER<kmax  % critério de paragem
    for i=2:N-1
        for j=2:N-1
            w(i,j)=(u(i-1,j)+u(i,j-1)+u(i,j+1)+u(i+1,j))/4;  % implementação da fórmula (4.22)
